<template>
  <div class="st-panel">
    ...
    <div>
      进度：{{ shiTi_index }}/{{ shiJuan.stTotal }} &nbsp;&nbsp;&nbsp;&nbsp;
      得分：{{ score }}
    </div>
    <hr />
    <el-row>
    ...
    <el-row>
      ...
      <!--      </el-checkbox-group>-->
      <el-radio-group
        v-if="shiTi.stOptionType == 'select'"
        v-model="userRadioOpetion"
      >
        <el-radio
...
export default {
  ...
  setup() {
    ...
    const shiJuanList =
      getCurrentInstance()?.appContext.config.globalProperties.$shijuan
    const shiTiList =
      getCurrentInstance()?.appContext.config.globalProperties.$shiti
    ...
    return {
      ...
      pre() {
        clear()
        console.info(
          'pre sj_no:' + data.shiJuan_no + ',st_index:' + data.shiTi_index
        )
      ...
      next() {
        ...
        clear()
        console.info(
          'next sj_no:' + data.shiJuan_no + ',st_index:' + data.shiTi_index
        )
        getShiTi(data.shiJuan_no, data.shiTi_index + 1)
      },
    }
